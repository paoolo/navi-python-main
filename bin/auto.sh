...
then
    ...
    if [ -z "${_APP_DAEMON}" ];
    then
        ${__dir}/__envi/bin/python -u ${PYTHONPATH}/navi/auto.py ${_APP_ARGS}
    else
        mkdir -p ${__dir}/__logs
        nohup ${__dir}/__envi/bin/python -u ${PYTHONPATH}/navi/auto.py ${_APP_ARGS} 0<&- &> ${__dir}/__logs/output.log &
    fi
else
